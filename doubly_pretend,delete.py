...
class LinkedList:
    ...
    def delete(self, node_to_delete):
        ...
        # 여기에 코드를 작성하세요
        if node_to_delete is self.head and node_to_delete is self.tail: # 경우1: 삭제할 노드가 마지막 남은 노드일때
            self.head = None
            self.tail = None

        elif node_to_delete is self.head: #경우2: 삭제할 노드가 head 노드일 때
            self.head = self.head.next
            self.head.prev = None

        elif node_to_delete is self.tail: #경우3: 삭제할 노드가 tail 노드일 때
            self.tail = self.tail.prev
            self.tail.next = None   
        
        else: #경우4: 중간에 있는 노드 삭제
